...
public class BitList {
    ...
    public BitList( String s ) {
        ...
        String[] bits = s.split("");
        BitListIterator iter = new BitListIterator();
        for (int i = bits.length -1; i>=0; i--) {
            if(Integer.parseInt(bits[i]) != ZERO && Integer.parseInt(bits[i]) != ONE)
                throw new IllegalArgumentException("Must be of Binary Radix!");
            if (i ==bits.length-1){
                iter.add(Integer.parseInt(bits[i]));
            }else{
                while(iter.hasNext())
                    iter.next();
            iter.add(Integer.parseInt(bits[i]));

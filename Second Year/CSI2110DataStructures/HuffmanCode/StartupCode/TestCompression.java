...
public static void main(String[] args) throws IOException, ClassNotFoundException { 
  	 Huffman myHuff=new Huffman();
  	 /* ArrayList<Integer> freqTable = myHuff.buildFrequencyTable(new FileInputStream("StartupCode/ACTGhandout.txt"));
	   myHuff.buildEncodingTree(freqTable);
	   int count = 0;
	
	   System.out.println(freqTable);
	   myHuff.encode("StartupCode/ACTGhandout.txt", "StartupCode/ACTGhandout.huf");
	   myHuff.decode("StartupCode/ACTGhandout.huf","a.txt"); */
  	 if (args.length==0) { // hardcoded and interactive inputs
  		 // hardcoded test for file genes.txt
  		 myHuff.encode("ACTGhandout.txt", "ACTGhandout.huf");
  		 myHuff.decode("ACTGhandout.huf", "ACTGhandoutRecovered.txt");
